...
class LabelPrint():
  def __init__(self, LabelInfo: LabelInfo) -> None:
  ...
  def draw_mwm_label_elements(self, image: Image, draw: ImageDraw, attributes: list[dict]) -> None:
    ...
    for attribute in attributes:
      if 'width' in attribute:
        ...
        options: dict = {"module_width": attribute['width'], "module_height": 8, "quiet_zone": 1, "write_text": False}
        barcode.write(barcode_buffer, options)
        barcode_image: Image = Image.open(barcode_buffer)
        image.paste(barcode_image, attribute['xy'])
  ...
  def create_label(self) -> None:
    ...
    attr: list[dict] = [
      ...
      {'xy': (self.mm_to_px(5), self.mm_to_px(48)), 'text': f'DESCRICÃO: {self.wrap_text(self.label_info.description)}', 'font': ImageFont.truetype(FONT_PATH, size=62)},
      {'xy': (self.mm_to_px(5), self.mm_to_px(68)), 'text': f'{self.label_info.barcode}', 'font': ImageFont.truetype(FONT_PATH, size=62), 'width': 0.34, 'height': 10},  
      {'xy': (self.mm_to_px(5),self.mm_to_px(90)), 'text': f'QUANTIDADE: {self.label_info.quantity}', 'font': ImageFont.truetype(FONT_PATH, size=62)},
      {'xy': (self.mm_to_px(70), self.mm_to_px(86)), 'text': f'{self.label_info.quantity}', 'font': ImageFont.truetype(FONT_PATH, size=62), 'width': 0.4, 'height': 8},
  ...
  def create_mwm_label(self) -> None:
    ...
    layout_positions: dict[str, tuple] = {
      'rectangle': (self.mm_to_px(7.5), self.mm_to_px(5.5), self.mm_to_px(97.5), self.mm_to_px(69.5)),
      'lines': [
    ...
    attributes: list[dict] = [   
      {'xy': (self.mm_to_px(7.5), self.mm_to_px(2.7)), 'text': 'MWM MOTORES E GERADORES', 'font': ImageFont.truetype(f'{FONT_PATH}YUGOTHB.TTC', size=self.mm_to_px(9, 100))}, # Cliente
      {'xy': (self.mm_to_px(88), self.mm_to_px(2.7)), 'text': 'V.2.3.4', 'font': ImageFont.truetype(f'{FONT_PATH}YUGOTHB.TTC', size=self.mm_to_px(9, 100))}, # Versão
      {'xy': (self.mm_to_px(8), self.mm_to_px(5.5)), 'text': 'Part:', 'font': ImageFont.truetype(f'{FONT_PATH}LUCON.TTF', size=self.mm_to_px(10, 100))}, # Part:
      {'xy': (self.mm_to_px(23), self.mm_to_px(9)), 'text': self.label_info.client_code, 'font': ImageFont.truetype(f'{FONT_PATH}CONSOLA.TTF', size=self.mm_to_px(27, 110))}, # Part_number
      {'xy': (self.mm_to_px(23), self.mm_to_px(18.3)), 'text': self.label_info.client_code, 'width': 0.22}, # Part_number_barcode
      {'xy': (self.mm_to_px(24), self.mm_to_px(17)), 'text': 'fornecedor/fabricante', 'font': ImageFont.truetype(f'{FONT_PATH}YUGOTHB.TTC', size=self.mm_to_px(6, 100))}, # fabricante/fornecedor
      {'xy': (self.mm_to_px(8), self.mm_to_px(28.5)), 'text': 'Supplier:', 'font': ImageFont.truetype(f'{FONT_PATH}LUCON.TTF', size=self.mm_to_px(8, 100))}, # Supplier:
      {'xy': (self.mm_to_px(23), self.mm_to_px(28.7)), 'text': '15175', 'font': ImageFont.truetype(f'{FONT_PATH}YUGOTHB.TTC', size=self.mm_to_px(7.2, 100))}, # Supplier_number
      {'xy': (self.mm_to_px(70), self.mm_to_px(28)), 'text': 'Date:', 'font': ImageFont.truetype(f'{FONT_PATH}YUGOTHB.TTC', size=self.mm_to_px(10, 100))}, # Date:
      {'xy': (self.mm_to_px(80), self.mm_to_px(28.5)), 'text': self.label_info.date, 'font': ImageFont.truetype(f'{FONT_PATH}YUGOTHR.TTC', size=self.mm_to_px(8, 100))}, # Data doa dia
      {'xy': (self.mm_to_px(8), self.mm_to_px(31.7)), 'text': 'Qty:', 'font': ImageFont.truetype(f'{FONT_PATH}LUCON.TTF', size=self.mm_to_px(10, 100))}, # Qty:
      {'xy': (self.mm_to_px(74), self.mm_to_px(34)), 'text':  str(self.label_info.quantity), 'font': ImageFont.truetype(f'{FONT_PATH}CONSOLA.TTF', size=self.mm_to_px(22.9, 110))}, # Qty_number
      {'xy': (self.mm_to_px(23), self.mm_to_px(32)), 'text': str(self.label_info.quantity), 'width': 0.22}, # Qty_number_barcode
      {'xy': (self.mm_to_px(84), self.mm_to_px(35)), 'text': 'PCs', 'font': ImageFont.truetype(f'{FONT_PATH}DUBAI-BOLD.TTF', size=self.mm_to_px(12.5, 110))}, # PCs
      {'xy': (self.mm_to_px(8), self.mm_to_px(43)), 'text': 'Lot:', 'font': ImageFont.truetype(f'{FONT_PATH}LUCON.TTF', size=self.mm_to_px(10, 100))}, # Lot:
      {'xy': (self.mm_to_px(23), self.mm_to_px(43)), 'text': self.label_info.op, 'font': ImageFont.truetype(f'{FONT_PATH}YUGOTHB.TTC', size=self.mm_to_px(8.2, 100))}, # Lot_number
      {'xy': (self.mm_to_px(22.5), self.mm_to_px(45)), 'text': self.label_info.op, 'width': 0.28}, # Lot_number_barcode
      {'xy': (self.mm_to_px(8), self.mm_to_px(56)), 'text': 'ID:', 'font': ImageFont.truetype(f'{FONT_PATH}LUCON.TTF', size=self.mm_to_px(10, 100))}, # ID:
      {'xy': (self.mm_to_px(23), self.mm_to_px(56)), 'text': f'15175{self.label_info.mwm_date}{str(self.label_info.boxes).zfill(6)}', 'font': ImageFont.truetype(f'{FONT_PATH}YUGOTHB.TTC', size=self.mm_to_px(8.2, 100))}, # ID_number
      {'xy': (self.mm_to_px(22.5), self.mm_to_px(58)), 'text': f'15175{self.label_info.mwm_date}{str(self.label_info.boxes).zfill(6)}', 'width': 0.205}, # ID_number_barcode
    ]
  ...
  def print_label(self, file_path: str = './tmp/etq.png') -> None:
    ...
    try:
      win32print.StartDocPrinter(printer, 1, ("Print ETQ", None, "RAW"))
